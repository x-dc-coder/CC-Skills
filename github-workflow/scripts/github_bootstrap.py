#!/usr/bin/env python3
"""Bootstrap local git projects, plan layered commits, and connect GitHub remotes via gh CLI.

设计原则：GitHub 侧能力（创建仓库、用户信息、Issue、PR）全部走官方 gh CLI；
本工具只做"本地 git 流程的优化与扩展"：初始化、预览、安全扫描、提交信息生成、
review manifest、分层提交建议与发布。所有业务逻辑在 lib/ 模块中，本文件是薄 CLI。
"""
...
from lib.common import BootstrapError, log_info
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Bootstrap Git and GitHub setup for local projects (gh CLI)"
    )
    ...
    profile_parser.add_argument("--description", default=None, help="个人简介（bio）")
    profile_parser.add_argument("--company", default=None, help="公司")
    profile_parser.add_argument("--location", default=None, help="所在地")
    profile_parser.add_argument("--website", default=None, help="个人网站（blog）")
...
def emit(result: Dict[str, Any], as_json: bool) -> None:
    ...
    if result.get("remote"):
        ...
        print(f"remote: {remote.get('remote_name')} -> {remote.get('url')} [{remote.get('action')}]")
