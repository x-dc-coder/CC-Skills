...
3. **生成模块图**
   ...
   - 输出路径：`~/.claude/skills-output/diagram-module/module-diagram.png`（默认，可自定义）
...
~/.claude/skills-output/diagram-module/    # 生成的模块图 PNG（默认输出目录）
```
...
默认输出路径由 CLI 自动推断（绝对路径输入 → `<项目目录>/thesis-output/diagram-module/module-diagram.png`，否则回退到 `~/.claude/skills-output/diagram-module/module-diagram.png`）
