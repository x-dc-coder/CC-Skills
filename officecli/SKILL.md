...
以下细节已移至 `references/advanced-operations.md`，仅在对应任务需要时读取：

- add/clone 与各格式元素类型矩阵
- xlsx Pivot、排序与文本锚定插入
- move/swap/remove
- 原子 batch、dump/replay
- raw XML / raw-set / add-part

优先运行 `officecli help <format> <element>` 获取当前 CLI 的权威属性；不要凭静态清单猜字段。
