...
description: Diagramming know-how for Artifacts — how to draw inline-SVG diagrams (architecture, flow, mechanism, state, comparison) that show the real mechanism and stay legible in both light and dark themes. Use this skill whenever a picture earns its place in an HTML page or artifact: depicting how data flows, which components talk, what changes between two options, what state a request moves through, or any mechanism a cold reader would otherwise have to assemble from prose. Covers when a diagram is worth drawing at all, what to show vs leave out, and the inline-SVG mechanics (viewBox scaling, currentColor theming, arrowhead markers, text sizing, grid alignment, figure/caption, self-containment). Triggers on: "draw a diagram", "architecture diagram", "flow diagram", "机制图", "架构图", "画个图", "inline SVG diagram", "show how X works".
---
